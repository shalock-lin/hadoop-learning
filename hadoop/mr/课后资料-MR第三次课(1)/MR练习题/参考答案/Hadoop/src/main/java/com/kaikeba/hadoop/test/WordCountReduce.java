 ...
 * reduce方法的输入的键的类型kin、值的类型vin；输出的键的类型kout、输出的值的类型vout
 * 因为map的输出作为reduce的输入，所以此处的kin、vin类型分别与map的输出的键类型、值类型相同
...
public class WordCountReduce extends Reducer<Text, IntWritable, Text, IntWritable> {
     ...
     * 同样，key是spark的键值对被分成一组；merge成[spark, Iterable(1,1,1)]，调用一次reduce方法
     *
     * @param key 当前组的key
